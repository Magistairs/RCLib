...
void DefaultTestRunner::AddTest(std::string_view name, std::function<bool()> testFunc)
{
	...
	auto it = std::find_if(m_tests.begin(), m_tests.end(), [&name](const Test& test) { return test.name == name; });
	...
	m_tests.emplace_back(Test{.name = std::string(name), .func = std::move(testFunc), .passed = false, .executed = false});
}
